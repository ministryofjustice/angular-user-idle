    ...
    startWatching() {
        ...
        this.idleSubscription = this.idle$
            .pipe(
                ...
                filter(arr => !this.isTimeout), // firstly if timeout has happened then do nothing
                tap((events) => {
                    // if there are events, then the user is no longer idle
                    if (events && events.length) {
                        // stop the timer if it has already started
                ...
                filter(
                    // filter out any events, and only if idle is not already detected and the activity timer is not set
